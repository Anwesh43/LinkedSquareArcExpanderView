...
fun Canvas.drawExpanderPath(r : Float, scale : Float, paint : Paint) {
    val sd : Int = 180
    val ed : Int = 360
    val path : Path = Path()
    for (j in sd..ed) {
        val x : Float = r * Math.cos(j * Math.PI / 180).toFloat()
        val y : Float = r * scale * Math.sin(j * Math.PI / 180).toFloat()
        if (j == sd) {
            path.moveTo(x, y)
        } else {
            path.lineTo(x, y)
...
fun Canvas.drawSAENode(i : Int, scale : Float, paint : Paint) {
    ...
    for (j in 0..(lines - 1)) {
        ...
        rotate(rotDeg * j)
        translate(0f, -size / 2)
        drawExpanderPath(size / 2, sc1.divideScale(j, lines), paint)
        restore()
    }
    restore()
}
...
class SquareArcExpanderView(ctx : Context) : View(ctx) {
    ...
    data class SquareArcExpander(var i : Int) {

        private val root : SAENode = SAENode(0)
        private var curr : SAENode = root
        private var dir : Int = 1
